<template>
        ...
        >
          ...
          <template #overlay>
            <a-menu class="!p-2 w-96 max-h-80 overflow-y-auto">
              ...
              <a-menu-item v-if="announcements.length === 0">
  ...
  <Announcement
    v-if="openAnnouncement && selectedAnnouncement"
...
import {
  logout,
  teacherChangeAccountPassword,
  searchUser,
  teacherAccount,
  teacherGetAnnouncementCurrentWeek,
  teacherGetAllAnnouncements,
} from "@/services/api";
...
const handleClickAnnouncement = () => {
  if (isFirstLoad.value) {
    if (announcements.value.length >= pagination.value.pageSize) {
      pagination.value.page += Math.floor(
        announcements.value.length / pagination.value.pageSize
      );
    }
  ...
  get(teacherGetAllAnnouncements, {
...
onMounted(() => {
  ...
  get(teacherGetAnnouncementCurrentWeek).then((res) => {
    if (res.code !== 200) {
      toast.error("Lấy thông báo thất bại");
      return;
    }
