...
import { adminClass, adminSearchStudent } from "@/services/api";
...
const { open, classId } = defineProps<{
  open: boolean;
  reload: boolean;
  classId: number;
}>();
...
const handleOk = () => {
  ...
  toast.promise(
    post(adminClass+ `/${classId}` + "/students/add/batch", {
      studentIds: selectedStudents.value.map((student) => student.studentId)
    }).then((res) => {
      if (res.code !== 201) {
        throw new Error(res.error || "Thêm học sinh thất bại");
      }
    ...
    }),
    {
      ...
      error: {
        render: (error: any) => {
          return error.data || "Thêm học sinh thất bại";
        },
      }
    }
  );
