...
const { open, classData } = defineProps<{
  open: boolean;
  classData: Class;
  reload: boolean;
}>();
...
// Update form state when classData changes
watch(
  () => classData,
  (newClassData) => {
    if (newClassData) {
      formState.className = newClassData.className;
      formState.teacherId = String(newClassData.teacherId);
      formState.subjectId = String(newClassData.subjectId);
      formState.startDate = newClassData.startDate
        ? dayjs(newClassData.startDate)
        : null;
      formState.endDate = newClassData.endDate
        ? dayjs(newClassData.endDate)
        : null;
      formState.description = newClassData.description || "";
      formState.tuition = newClassData.tuition;
      formState.tuitionDueDate = newClassData.tuitionDueDate
        ? dayjs(newClassData.tuitionDueDate)
        : null;
      formState.finalTermWeight = newClassData.finalTermWeight;
      formState.absenceWarningThreshold =
        newClassData.absenceWarningThreshold;
      formState.absenceLimit = newClassData.absenceLimit;
      formState.dayOfWeek = newClassData.dayOfWeek as DayOfWeek;
      formState.periodStart = newClassData.periodStart ;
      formState.periodEnd = newClassData.periodEnd ;
      formState.room = newClassData.room;

      // Set selected teacher
      selectedTeacher.value = {
        teacherId: newClassData.teacherId,
        username: "",
        fullName: newClassData.teacherFullName,
        email: newClassData.teacherEmail,
        avatarUrl: newClassData.teacherAvatarUrl,
      };

      // Set selected subject
      selectedSubject.value = {
        subjectId: newClassData.subjectId,
        subjectCode: newClassData.subjectCode,
        subjectName: newClassData.subjectName,
      };
    }
  },
  { immediate: true }
...
const handleOk = () => {
  formRef.value?.validate().then(() => {
    ...
    toast.promise(
      put(adminClass + `/${classData.id}`, submitData).then((res: any) => {
        if (res.code !== 202) {
          throw new Error(res.message || "Cập nhật lớp học thất bại");
        }
        emit("update:open", false);
        emit("update:reload", true);
      }),
      {
        pending: "Đang cập nhật lớp học...",
        success: "Cập nhật lớp học thành công!",
        error: {
          render: (error: any) => {
            return error.data.message || "Cập nhật lớp học thất bại";
          },
        },
      }
    );
