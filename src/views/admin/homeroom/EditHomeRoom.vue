...
import { get, post, put } from "@/services/callApi";
import { adminCreateHomeRoom, adminSearchTeacher, adminUpdateHomeRoom } from "@/services/api";
...
const { open, homeRoom } = defineProps<{
    open: boolean;
    homeRoom: HomeRoom;
    reload: boolean;
}>();
...
const selectedResult = ref<SearchResult | null>(homeRoom ? {
    teacherId: homeRoom.teacherId,
    username: "",
    fullName: homeRoom.teacherName,
    email: homeRoom.teacherEmail,
    avatarUrl: homeRoom.teacherAvatarUrl,
} : null);
...
const formState = reactive<FormState>({
  name: homeRoom ? homeRoom.name : "",
  teacherId: homeRoom ? homeRoom.teacherId.toString() : "",
});
  ...
  toast.promise(
    put(adminUpdateHomeRoom, {
      ...
      if (res.code === 202) {
        ...
        throw new Error("Cập nhật lớp chủ nhiệm thất bại");
      }
    }),
    {
      pending: "Đang cập nhật lớp chủ nhiệm...",
      success: "Cập nhật lớp chủ nhiệm thành công!",
      error: "Cập nhật lớp chủ nhiệm thất bại!",
    }
  );
};
</script>
